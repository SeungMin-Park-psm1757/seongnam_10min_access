...
- `data/processed/data_manifest.json`에는 원천 파일명, 기준시점, 출처 URL, 행 수, 산출 일자, 가중치가 기록된다.
- `data/processed/accessibility_metrics_audit.csv`에는 점수 산정 전 의료기관 수, 약국 수, 가장 가까운 노인복지관 거리, 가장 가까운 버스정류장 거리가 남는다.
- 캡처 PDF를 생성하면 `public/reports/v2-actual-data-captures/capture_manifest.json`에 캡처 URL, viewport, 생성 시각, 커밋 해시가 기록된다.
