...
def build():
    ...
    add_form_row(
        ...
        [
            ...
            "성남시 버스공영차고지 현황(data.go.kr): 교통 기반시설 대리 지표 산정",
    ...
    add_form_row(
        ...
        [
            ...
            "노인복지관은 공식 시설 목록과 성남시 좌표 PDF를 결합했으며, 교통은 공영차고지 거리 기반 대리 지표로 표시했다.",
        ],
    )
    add_form_row(
        table,
    ...
    add_form_row(
        table,
        "활용방안 및 기대효과",
        "보건소, 교통, 복지 부서가 같은 화면에서 우선 점검 생활권을 공유할 수 있다. 향후 버스정류장 전수 좌표, 보행 네트워크, 통신·카드 집계 데이터가 추가되면 실제 10분 도달성과 이용 수요를 결합한 정책 의사결정 도구로 확장할 수 있다.",
    )
    ...
    sources = [
        ...
        ("버스공영차고지", "https://www.data.go.kr/data/15061124/fileData.do"),
    ...
    notes = [
        ...
        "교통 지표는 버스정류장 전수 접근성이 아니라 공영차고지 거리 기반 대리 지표다. 발표 시 이 부분을 한계로 명시하고, 정류장 좌표 확보 후 재산출한다고 설명한다.",
